...
class MeituanSpider(object):
    ...
    fieldKey = ['店铺名称', '页面id', '类别', '品牌名称', '品牌id', '品牌logo', '评分',
                '平均价格', '最低价格', '所属地区', '城市Id', '纬度', '经度', '详细地址',
                '楼层', '地铁站id', '停车信息', '优惠套餐情况', '营业时间', '联系电话',
                '餐厅简介', '特色菜', '是否小吃', '有无外卖', '上周订单数',
                '历史订单数', 'wifi', '支持预定']

    # 美团美食爬虫
    def __init__(self, saveMode='txt', cityId=1, i=0, acquiredCount=0):
        '''
    ...
    def run(self):
        try:
            while True:
                url = self.baseUrl.format(self.cityId, str(self.i * limit), limit)
                print('************ %s *************' % self.i)
                ...
                for item in itemlist:
                    self.save_item(item)
                self.acquiredCount += len(itemlist)
                print('已成功请求%d个商家信息' % ((self.i + 1) * limit))
                print('已成功获取%d个商家信息' % (self.acquiredCount))
                self.i += 1
...
if __name__ == '__main__':
    for j in range(0, 1501):
        spider = MeituanSpider(saveMode='mongodb', cityId=j)
        # spider = MeituanSpider(saveMode='csv', cityId=j)
        print('city %s is start!' % j)
        spider.run()
        print('city %s is end!' % j)
